...
Codex runs with `--json`, and the Go runner returns as soon as the final `agent_message` item completes. The translated text is already available at that point, so the CLI does not wait for later Codex teardown.
