...
- Replaced the npm Go-binary wrapper with a TypeScript implementation compiled to `npm/dist/t.js`.
...
Follow-up: the repository later removed the Go implementation entirely and switched GitHub Release/Homebrew artifacts to Bun single-file executables generated from the same TypeScript source. The npm package still ships the Node.js `dist/t.js` entrypoint.

## Verification

- `npm test --prefix npm` passes against the existing fake Codex and Claude CLIs.
- `node npm/dist/t.js ja hello` works against real Codex.
