...
const JSON_SCHEMA =
  '{"type":"object","properties":{"translated_text":{"type":"string"}},"required":["translated_text"],"additionalProperties":false}';
const MAX_CODEX_PROMPT_ARG_BYTES = 16 * 1024;

type ExitCode = 0 | 1 | 2 | 3 | 4 | 5 | 6;
type TranslationMode = "auto_pair" | "target";
...
class AppError extends Error {
  ...
  constructor(code: ExitCode, message: string, hint = "") {
    super(message);
    this.code = code;
    this.hint = hint;
  }
...
function applyPositional(inv: Invocation, positional: string[]): void {
...
async function loadConfig(configPath: string): Promise<{ config: Config; exists: boolean }> {
  ...
  try {
    ...
    throw withHint(new AppError(3, `failed to parse config: ${(error as Error).message}`), configPath);
  }
...
function parseConfig(config: Config, data: string): void {
  let section = "";
...
function parseRootConfigValue(config: Config, key: string, value: string): void {
  switch (key) {
    case "version":
...
async function needsWizard(config: Config, exists: boolean): Promise<boolean> {
  if (!exists) {
    return true;
  }
...
async function runWizard(configPath: string, initialConfig: Config): Promise<Config> {
  ...
  if (tools.length === 0) {
    throw withHint(
      new AppError(4, "no supported Agent CLI found"),
  ...
  const rl = scriptedAnswers === undefined
    ...
    : undefined;
  const askSetup = async (prompt: string) => {
    if (scriptedAnswers !== undefined) {
      process.stderr.write(prompt);
      return (scriptedAnswers.shift() ?? "").trim();
    }
    return ask(rl as ReturnType<typeof createInterface>, prompt);
  };
  try {
...
async function selectTool(
  askTool: (prompt: string) => Promise<string>,
...
function selectedTool(inv: Invocation, config: Config): string {
  if (inv.tool !== "") {
    return inv.tool;
  }
...
function detectAll(runtime: DetectionRuntime): DetectionResult[] {
  return [codexAdapter, claudeAdapter].map((adapter) => adapter.detect(runtime));
}

function recommendedTool(results: DetectionResult[]): DetectionResult | undefined {
  return results
...
type Adapter = {
  id: "codex" | "claude";
  displayName: string;
  detect(runtime: DetectionRuntime): DetectionResult;
  buildCommand(req: TranslationRequest, runtime: RuntimeContext): ExecSpec;
  extractResult(raw: ExecResult): { text: string };
};
...
function byID(id: string): Adapter | undefined {
  if (id === "codex") {
    return codexAdapter;
  }
  if (id === "claude") {
...
function runStreamingJSON(spec: ExecSpec, id: string, timeoutMS: number): Promise<ExecResult> {
...
function runCommand(spec: ExecSpec, id: string, timeoutMS: number): Promise<ExecResult> {
  return new Promise((resolve, reject) => {
    const child = spawn(spec.command, spec.args, {
      cwd: spec.workDir || undefined,
      stdio: [spec.stdin === "" ? "ignore" : "pipe", "pipe", "pipe"]
...
function jsonAgentMessage(line: string): string | undefined {
  try {
...
function buildPlainTextPrompt(req: TranslationRequest): string {
  const modeInstruction = buildModeInstruction(req);
...
function buildModeInstruction(req: TranslationRequest): string {
...
function normalizeAllowRaw(raw: string): { text: string } {
...
function parseEmbeddedJSON(raw: string): { text: string } | undefined {
...
function parseJSONTranslation(raw: string): { text: string } | undefined {
  let body: unknown;
  try {
    body = JSON.parse(raw);
  } catch {
...
function mustLanguage(code: string): Language {
  return resolveLanguage(code) ?? { code, name: code };
}

function normalizeLanguageKey(input: string): string {
  return input.trim().replaceAll("_", "-").toLowerCase();
}

function detectLocalLanguage(): Language {
...
function localePrefix(value: string): string {
...
function defaultPairLanguage(local: Language): Language {
  if (local.code === "en") {
...
function score(
  ...
  let total = 0;
  if (found) total += 50;
  if (authenticated) total += 30;
  if (nonInteractive) total += 20;
  if (runtime.existingDefault === id) total += 100;
  if (runtime.envTool === id) total += 100;
  return total;
}
...
function agentRunError(id: string, code: number | null, signal: NodeJS.Signals | null, stderr: string): AppError {
...
function runErrorMessage(status: string, stderr: string): string {
...
function withHint(error: AppError, hint: string): AppError {
...
function parseString(value: string, key: string): string {
  try {
...
function isNoTranslationError(error: unknown): boolean {
...
function isDirectExecution(entryFile: string): boolean {
